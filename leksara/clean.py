...
def _clean_one(
	...
	*,
	...
	if reduce_repeat:
		if remove_punct:
			# Bebaskan: kurangi semua karakter berulang termasuk tanda baca
			t = normalize_repeated(t, max_repeat=max_repeat)
		else:
			# Jangan sentuh tanda baca: hanya kurangi pengulangan alfanumerik
			pattern = re.compile(r"([A-Za-z0-9])\1{" + str(max_repeat) + r",}")
			t = pattern.sub(lambda m: m.group(1) * max_repeat, t)
